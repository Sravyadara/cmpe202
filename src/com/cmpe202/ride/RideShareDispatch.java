...
public class RideShareDispatch implements DispatchStrategyInterface{
	...
	public Driver searchDriver(Ride ride) throws SQLException {
				...
				for(Map.Entry<String,Point> entry : driverCoordinates.entrySet()){
					distances.put(entry.getKey(), Math.sqrt(Math.pow((entry.getValue().getX() - customerPoint.getX()), 2) + Math.pow((entry.getValue().getY() - customerPoint.getY()), 2)));
				}
				...
				int status = dispatchDAO.insertDispatch(ride.getRideid(), driver.getMemberid(), driver.getVehicleId(), member_emailId);
				
				if(status != 0){
					//notify customer and driver
					System.out.println("Customer-Notification:-----Your ride will reach you in "+totalTime*5 + " mins");
					System.out.println("Driver-Notification:-----Your ride is waiting at "+ride.getPickuplocation() + " with Customer email: "+member_emailId);
					}
	...
	private static Map<String,Double> sortByComparator(Map<String,Double> distances) {

		// Convert Map to List
		List<Map.Entry<String,Double>> list = 
			new LinkedList<Map.Entry<String,Double>>(distances.entrySet());
