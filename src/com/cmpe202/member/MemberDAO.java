...
public class MemberDAO {
    ...
    //to be used when the driver from member table has to be inserted into drivers table with vehicleId assigned. - ONLY FOR TAXI driver_type
    public int insertDriver(String driverId, String vehicleId, String currentLocation, String driverAvailabilityStatus, String driver_type) throws SQLException{
        
        
        String query = "INSERT INTO Driver(driver_emailid,driver_vehicleid,driverlocation,driverstatus,driver_type) VALUES "
                + "( "+driverId+ ","+vehicleId+","+ currentLocation + ","+ driverAvailabilityStatus + "," + driver_type+ ","+")";
        int status = 0;
        try {
            connection = ConnectionFactory.getConnection();
            statement = connection.createStatement();
  ...
  //to be used for taxi driver by by assigning a free vehicle from vehicle table
    ...
    //to be used for admin to add new vehicle to inventory
    public int AddVehicle(String requestNoOfSeats, String vehicle_type, String vehicle_model, String vehiclespecification) throws SQLException{
        String query = "INSERT INTO vehicle(noofseats,vehiclemodel,vehicletype,vehiclespecification) VALUES "
                + "( "+requestNoOfSeats+ ","+vehicle_model+","+ vehicle_type + ","+ vehiclespecification+ ")";
        int status = 0;
        try {
            connection = ConnectionFactory.getConnection();
            statement = connection.createStatement();
